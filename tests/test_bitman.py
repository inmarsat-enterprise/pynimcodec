...
@pytest.mark.parametrize('test_val', [-1, 0, 5, -5])
def test_bitarray_from_int(test_val):
    bit_array = BitArray.from_int(test_val)
    assert isinstance(bit_array, BitArray)
    # special case: 0 has bit_length() = 0, but still uses at least 1 bit
    expected_len = max(1, test_val.bit_length() + (1 if test_val < 0 else 0))
    assert len(bit_array) == expected_len
